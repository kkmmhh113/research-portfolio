...
당시에는 앙상블과 하이퍼파라미터 조정으로 큰 성능 향상을 얻지 못했던 것으로 기억합니다. 당시의 비교 기록과 Optuna 탐색 결과가 모두 남아 있지는 않아 정확한 개선 폭을 다시 확인하기는 어렵습니다.

당시 실행 기록에는 테스트 앙상블 정확도 86.93%가 남아 있습니다. 다만 코드를 다시 정리하는 과정에서 체크포인트 순위를 관리하는 문제가 발견됐고, 당시 체크포인트 파일도 남아 있지 않습니다. 의도한 모델 조합으로 나온 점수인지 확인하기 어려워 이 수치는 과거 실행 기록으로만 남겼습니다. 현재 공개한 코드는 해당 문제를 수정한 버전이며, 아직 다시 학습하지 않았습니다. 자세한 내용은 [결과 기록](results/historical_results.json)과 [코드 변경 기록](revision-notes.md)에 정리했습니다.
...
2025년에 사용한 학습 코드를 2026년에 다시 정리했습니다. 데이터 준비·학습·평가 단계를 나누고 체크포인트 저장 방식을 수정했습니다. 앞으로 정규화 값의 출처와 전체 실행 과정을 확인할 예정입니다. 기본 학습 코드와 Optuna 탐색 코드는 학습률 스케줄러도 달라서, 결과 차이를 Optuna의 효과만으로 해석하기는 어렵습니다.
